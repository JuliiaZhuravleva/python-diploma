...
- **Django Baton** - современная админ-панель с Bootstrap 5
...
Для корректной работы асинхронных задач (отправка email уведомлений) необходимо запустить Celery worker в отдельном терминале:

```bash
# В новом терминале (с активированным виртуальным окружением)
celery -A order_service worker --loglevel=info
```
...
Проект использует Django Baton для современного интерфейса администрирования с улучшенным UX/UI.

### Особенности админки:
- **Современный дизайн** на базе Bootstrap 5 и Material Symbols
- **Структурированное меню** с логическими группами (пользователи, каталог, заказы)
- **Responsive дизайн** для работы на всех устройствах
- **Быстрая навигация** по всем моделям системы
- **Поддержка тем** (автоматическое переключение светлая/темная)
...
3. Выполните запросы в следующем порядке:
    ...
    - Подтверждение email (токен будет отправлен на email при настроенной отправке или выведен в логи Celery)
